...
class MetaModule(nn.Module):
    ...
    def update_params(self, lr_inner, lr_base,source_params=None,
                      solver='adam', beta1=0.9, beta2=0.999, weight_decay=5e-4):
        if solver == 'sgd':
            ...
            for i,(tgt,gradVal) in enumerate(zip(self.named_params(self), source_params)):
                name_t, param_t = tgt
                ...
                if i>=159 :
                    step_size = lr_base / bias_correction1
    ...
    def copyModel(self, newModel, same_var=False):
        ...
        # copy vars
        # for name, param in zip(metaName[0:-2], partW[0:-2]):
...
class MetaResNet(MetaModule):
    ...
    def __init__(self, dropout=0.1, num_classes=0, cut_at_pooling=False, norm=True,num_features=2048):
        ...
        self.feat_bn = MetaBatchNorm1d(self.num_features)
        self.relu = nn.ReLU(inplace=True)
