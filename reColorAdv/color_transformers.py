...
class FullSpatial(ParameterizedTransformation):
    ...
    def forward(self, imgs):
        ...
        params_list = params_list.expand(imgs.shape[0], -1, -1)
                ...
                for delta_z in [0, 1]:
                    ...
                    for color_index, resolution in [
                        ...
                        color_index = color_index.clamp(
                            0, resolution - 1) #64 *256 *128
                        params_index = (params_index * resolution +
                                        color_index) # 64 *256*128

                    params_index = params_index.view(imgs.shape[0], -1)[:, :, None] \
                        .expand(-1, -1, 3)   # 64 *32768 * 3


                    vertex_values.append(
                        params_list.gather(1, params_index)
                        .view(imgs.shape[0], W, H, C)
                    )
