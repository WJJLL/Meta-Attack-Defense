...
class DeltaAddition(AdversarialPerturbation):
    ...
    def setup(self, x):
        ...
        self.delta = nn.Parameter(torch.zeros((1,3, 256, 128)).cuda())
        self.initialized = True
    ...
    def make_valid_image(self, x):
        ...
        change_in_delta=change_in_delta.mean(dim=0).unsqueeze(0)
    ...
    def forward(self, x):
        ...
        current_noise = self.delta
        current_noise = F.interpolate(current_noise, mode=MODE, size=tuple(x.shape[-2:]),
                                      align_corners=True).squeeze()
        return torch.clamp(x + current_noise, 0, 1)
##############################################################################
#                                                                            #
#                               SPATIAL PARAMETERS                           #
#                                                                            #
